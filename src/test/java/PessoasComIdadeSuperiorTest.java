...
public class PessoasComIdadeSuperior {
    public static void main(String[] args) throws IOException, InterruptedException {
        try {
            HttpClient client = HttpClient.newHttpClient();
            int idadeAlvo = 50;
            String baseUrl = "http://localhost:8080/pessoas/";
            String condicao = "com-idade-superior/";
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(baseUrl + condicao + idadeAlvo))
                    .build();

            HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());

            System.out.println("--- Obtendo resposta de /pessoas/"+ condicao + " ---");
